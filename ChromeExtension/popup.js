...
const fetchAcronymApi = async (search_term) => {
    ...
    let databaseino = await fetch('./database.json',Headers={'Content-Type': 'application/json'})
                .then(res => res.json())
                .then(data => {
                return data
                })
    ...
    const searchupp = await search_term.toUpperCase()

    console.log(await databaseino.body[await searchupp])
    let jsonobj = JSON.stringify(await databaseino)
    let jsonobj2 = JSON.parse(await jsonobj)

    console.log(await typeof(await databaseino))
    console.log(await typeof(await jsonobj))
    console.log(await typeof(await jsonobj2))

    console.log(jsonobj2.A)

    let GETBASE = []
    GETBASE = await databaseino.body[searchupp]
    ...
    let alldata = [].concat(await GETBASE, await results)
